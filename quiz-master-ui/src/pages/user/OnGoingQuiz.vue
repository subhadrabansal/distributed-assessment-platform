<template>
  <div class="flex-grow-1 p-4">
    <h2 class="mb-4 text-center">Ongoing Quizzes</h2>
    ...
    <div v-else-if="groupedQuizzes.length === 0" class="text-center py-8">
      ...
      <br />
      <button class="btn btn-primary mt-3 me-2" @click="goToRegistered">Go to Registered Quizzes</button>
      <button class="btn btn-secondary mt-3" @click="goToUpcoming">Go to Upcoming Quizzes</button>
    </div>
    <div v-else>
            ...
            <div v-for="quiz in chapter.quizzes" :key="quiz.id" class="col-md-4">
              <div class="card h-100 shadow-sm-lg">
                <div class="card-body d-flex flex-column justify-content-between">
                  ...
                  <button class="btn btn-primary w-100 mt-3" @click="() => joinQuiz(quiz.id)">Join</button>
                </div>
              </div>
            </div>
          </div>
...
const fetchQuizzes = async (search = '') => {
  ...
  try {
    const url = search
      ? `/user/quiz/ongoing-unregistered/search?text=${encodeURIComponent(search)}`
...
const joinQuiz = async (quizId: number) => {
  try {
    await axios.post('/user/quiz/register', { quiz_id: quizId })
    flash.setFlash('Successfully joined the quiz!', 'success')
    fetchQuizzes(searchText.value.trim())
  } catch (e: any) {
    flash.setFlash(e?.response?.data?.error_message || 'Failed to join quiz', 'error')
  }
}
</script>
