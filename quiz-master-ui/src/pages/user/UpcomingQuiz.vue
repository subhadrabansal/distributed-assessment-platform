<template>
  <div class="flex-grow-1 p-4">
    <h2 class="mb-4 text-center">Upcoming Quizzes</h2>
    ...
    <div v-else-if="groupedQuizzes.length === 0" class="text-center py-8">
      ...
      <button class="btn btn-secondary mt-3" @click="goToOngoing">Go to Ongoing Quizzes</button>
    </div>
    <div v-else>
...
const fetchQuizzes = async (search = '') => {
  ...
  try {
    const url = search
      ? `/user/quiz/upcoming-unregistered/search?text=${encodeURIComponent(search)}`
