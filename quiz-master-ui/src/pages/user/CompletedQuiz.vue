<template>
  <div class="flex-grow-1 p-4">
    <h2 class="mb-4 text-center">Completed Quizzes</h2>
            ...
            <div v-for="quiz in chapter.quizzes" :key="quiz.id" class="col-md-4">
              <div class="card h-100 shadow-sm-lg">
                <div class="card-body d-flex flex-column justify-content-between">
                  ...
                  <hr/>
                  <button class="btn btn-success w-100 mt-2" @click="() => viewScore(quiz.id)">View</button>
                </div>
              </div>
            </div>
          </div>
...
const fetchQuizzes = async (search = '') => {
  ...
  try {
    const url = search
      ? `/user/quiz/completed/search?text=${encodeURIComponent(search)}`
      : '/user/quiz/completed'
